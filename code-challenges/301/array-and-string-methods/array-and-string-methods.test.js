...
const templateWithJQuery = () => {
  starWarsPeople.forEach(val =>{
    let variable1 = $('#template').clone();
    variable1.removeAttr('id')
    variable1.appendTo($('main'))
    variable1.find('h2').text(val.name)
    variable1.find('h3').text(val.height)
    variable1.find('p').text(val.eye_color)
  })
}
